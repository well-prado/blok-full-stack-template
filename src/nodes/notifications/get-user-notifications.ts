...
export default class GetUserNotifications extends NanoService<GetUserNotificationsInput> {
  ...
  async handle(ctx: Context, inputs: GetUserNotificationsInput): Promise<INanoServiceResponse> {
    ...
    try {
      ...
      const formattedNotifications = userNotifications.map((notification: { id: string; title: string; message: string; type: string; priority: string; readAt: Date | null; createdAt: Date; expiresAt: Date | null; actionUrl: string | null; actionLabel: string | null; category: string | null; metadata: string | null; sourceWorkflow: string | null; sourceNode: string | null; isRead?: boolean }) => ({
        id: notification.id,
