...
export default class HttpTrigger extends TriggerBase {
	...
	listen(): Promise<number> {
		...
		this.app.use(cookieParser()); // Parse cookies from incoming requests
		
		// CORS configuration for cross-origin requests (especially for Vite dev server)
		this.app.use(cors({
			origin: function (origin, callback) {
				// Allow requests from Vite dev server, production, and same-origin
		...
		/*
		 * Frontend React SPA Server - serves built frontend from /frontend/dist
		 */
		const frontendServer = require("../FrontendServer");
		this.app.use("/", frontendServer.default);
		 ...
		 */
			...
			this.app.use(["/api/:workflow", "/api/"], async (req: Request, res: Response): Promise<void> => {
				...
				await this.tracer.startActiveSpan(`${workflowNameInPath}`, async (span: Span) => {
					try {
						...
						// Check for custom cookie header from workflow context
						if (ctx.vars && ctx.vars.setCookieHeader && typeof ctx.vars.setCookieHeader === 'string') {
							res.setHeader("Set-Cookie", ctx.vars.setCookieHeader);
						}
